...
int DrmPlane::Init() {
  ...
  ret = drm_->GetPlaneProperty(*this, "IN_FORMATS", &format_property_);
  if (ret) {
    ALOGE("Could not get IN_FORMATS property");
    return ret;
  } else {
    ALOGE("get IN_FORMATS property");
  }
  ...
  ret = drm_->GetPlaneProperty(*this, "HDR_FD", &hdr_fd_);
  if (ret)
    ALOGI("Could not get hdr_fd property");

  ret = drm_->GetPlaneProperty(*this, "split", &virtual8k_split_);
  if (ret)
    ALOGI("Could not get virtual8k_split property");
  ...
  properties_.push_back(&format_property_);
...
const DrmProperty &DrmPlane::format_property() const {
...
const DrmProperty &DrmPlane::hdr_fd_property() const {
  return hdr_fd_;
}
